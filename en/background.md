# Background & Use Case - ENABLE ANC Interoperability IG v0.2.0
...
| 5 | Facility name | `Organization.name` | (none) |
...
Full clinical observations (weight, BP, urine, HIV status, delivery outcome), multi-facility referral, and physical-site detail (Location resource). See the [Change Log](changes.md) for the roadmap.

### Standards

FHIR R4 · WHO SMART Guidelines ANC (DAK) codes as the primary Observation codes· LOINC and SNOMED as optional cross-mappings · [ENABLE Ethiopia-local codes](CodeSystem-enable-et-anc.md) for the four fields with no WHO DAK code by design (MRN, DoB, phone, visit number).
