# Actors - ENABLE ANC Interoperability IG v0.2.0
...
The EMR **SHALL** send exactly one [ENABLE ANC Bundle](StructureDefinition-enable-anc-bundle.md) per woman, containing one [Patient](StructureDefinition-enable-anc-patient.md), one [Organization](StructureDefinition-enable-organization.md), one [LMP](StructureDefinition-enable-last-menstrual-period.md) Observation, one [EDD](StructureDefinition-enable-expected-delivery-date.md) Observation, at least one finished [Encounter](StructureDefinition-enable-anc-encounter.md) and, for every finished Encounter, one [Gestational Age](StructureDefinition-enable-gestational-age.md) and one [ANC Visit Number](StructureDefinition-enable-anc-visit-number.md) Observation. See [The Contract](contract.md) for exact cardinalities.
